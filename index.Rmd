...
```{r transform-demo, fig.cap = "Composite (A) neutral and (B) smiling faces made from 49 indvidual neutral and smiling identities. (C) Individual smiling faces were (D) averaged with the smiling composite or (E) transformed by 50% of the shape and color differences between the neutral and smiling composites (E)."}
...
You can automatically delineate faces with a simpler template (Fig.\ \@ref(fig:auto-delin)) using the online services provided through the free web platform Face++ [-@faceplusplus], or dlib models provided by [Davis King on a CC-0 license](https://github.com/davisking/dlib-models). 
...
```{r, , fig.cap="Left and right profile templates available via webmoprh.org."}
